...
with tab6:
    ...
    st.markdown("### 1. Gradient Values Over Iterations (Batch)")
...
with tab3:
    ...
    with btab:
        ...
        y_train_pred = m.predict(X_train,method="batch")
        y_val_pred = m.predict(X_val,method="batch")
    ...
    with mbtab:# Generate predictions
        y_train_pred = m.predict(X_train,method="mini-batch")
        y_val_pred = m.predict(X_val,method="mini-batch")
    ...
    with sgtab:# Generate predictions
        y_train_pred = m.predict(X_train,method="stochastic")
        y_val_pred = m.predict(X_val,method="stochastic")
...
st.header("Mini-Batch Metrics")

col1, col2, col3, col4 = st.columns(4)
with col1:
    st.metric("R² Score", f"{mini_batch_metrics['R^2']:.4f}")
with col2:
    st.metric("MSE", f"{mini_batch_metrics['MSE']:.4f}")
with col3:
    st.metric("RMSE", f"{mini_batch_metrics['RMSE']:.4f}")
with col4:
    st.metric("MAE", f"{mini_batch_metrics['MAE']:.4f}")

st.header("Stochastic Metrics")

col1, col2, col3, col4 = st.columns(4)
with col1:
    st.metric("R² Score", f"{stochastic_metrics['R^2']:.4f}")
with col2:
    st.metric("MSE", f"{stochastic_metrics['MSE']:.4f}")
with col3:
    st.metric("RMSE", f"{stochastic_metrics['RMSE']:.4f}")
with col4:
    st.metric("MAE", f"{stochastic_metrics['MAE']:.4f}")

# Interactive predictions
st.header("5️⃣ Make Predictions")
x_input = st.number_input("Enter a value for x:", value=35.0, step=1.0)
pred = m.predict(x_input)
mini_pred = m.predict(x_input,method="mini-batch")
stochastic_pred = m.predict(x_input,method="stochastic")

st.success(f"Predicted y value using Batch method: **{pred[0]:.4f}**")
st.success(f"Predicted y value using Mini-Batch method: **{mini_pred[0]:.4f}**")
st.success(f"Predicted y value using Stochastic method: **{stochastic_pred[0]:.4f}**")
...
with st.expander("📊 View Detailed Training History"):
    ...
    st.write(f"**Batch Final Slope (w):** {history['slope'][-1]:.6f}")
    st.write(f"**Batch Slope Change:** {history['slope'][-1] - initial_params['initial_slope']:.6f}")
    st.write(f"**Mini-Batch Final Slope (w):** {mini_batch_history['slope'][-1]:.6f}")
    st.write(f"**Mini-Batch Slope Change:** {mini_batch_history['slope'][-1] - initial_params['initial_slope']:.6f}")
    st.write(f"**Stochastic Final Slope (w):** {stochastic_history['slope'][-1]:.6f}")
    st.write(f"**Stochastic Slope Change:** {stochastic_history['slope'][-1] - initial_params['initial_slope']:.6f}")
    st.write("")
    st.write(f"**Initial Intercept (b₀):** {initial_params['initial_intercept']:.6f}")
    st.write(f"**Batch Final Intercept (b):** {history['intercept'][-1]:.6f}")
    st.write(f"**Batch Intercept Change:** {history['intercept'][-1] - initial_params['initial_intercept']:.6f}")
    st.write(f"**Mini-Batch Final Intercept (b):** {mini_batch_history['intercept'][-1]:.6f}")
    st.write(f"**Mini-Batch Intercept Change:** {mini_batch_history['intercept'][-1] - initial_params['initial_intercept']:.6f}")
    st.write(f"**Stochastic Final Intercept (b):** {stochastic_history['intercept'][-1]:.6f}")
    st.write(f"**Stochastic Intercept Change:** {stochastic_history['intercept'][-1] - initial_params['initial_intercept']:.6f}")
    st.write("")
    st.write(f"**Batch Final Gradient (Slope):** {history['grad_slope'][-1]:.6f}")
    st.write(f"**Batch Final Gradient (Intercept):** {history['grad_intercept'][-1]:.6f}")
    st.write(f"**Mini-Batch Final Gradient (Slope):** {mini_batch_history['grad_slope'][-1]:.6f}")
    st.write(f"**Mini-Batch Final Gradient (Intercept):** {mini_batch_history['grad_intercept'][-1]:.6f}")
    st.write(f"**Stochastic Final Gradient (Slope):** {stochastic_history['grad_slope'][-1]:.6f}")
    st.write(f"**Stochastic Final Gradient (Intercept):** {stochastic_history['grad_intercept'][-1]:.6f}")